...
const IndexAdmin=()=>{
    ...
    const categorieRes = function categorieRes(){
        ...
        for(var i=0;i<Rescategorie.length;i++ ){
            element.push(
                <option value={Rescategorie[i].id}>{Rescategorie[i].nom}</option>
            )
        }
        return element;
    }
    ...
    const data= function data(){
        ...
        element.push(
            <div className='row table'>
                ...
                <div className='inline-table'>Titre</div>
                <div className='inline-table'>Code</div>
             ...
             for(var i=0;i<resData.length;i++ ){
                 element.push( 
                    <div id={'del-'+resData[i].id} className='content-view-table row'>
                            ...
                            <div className='inline-table'><b>{gliphicon(resData[i].status)}</b></div>
                            <div className='inline-table'><b>{gliphicon(resData[i].add_home)}</b></div>
                            <div className='inline-table'>{btnActive(resData[i].status,resData[i].id)} <Link to={'/administration/coupons-update/'+resData[i].id} className='btf btn-success'>Modifier</Link></div>
                    </div>
                 )
             }       
             return element;
    ...
    const checkio = function checkio(io,e){
        ...
        const addHome = function addHome(){
            ...
            const url= localhost+'/controleur.php?p=CouponsAddHome'; 
            axios.post(url,formData)
            .then((res)=>{
  
                    if(res.data == 'coupons-add-home-success'){
        ...
        const RemoveHome = function RemoveHome(){
            ...
            const url= localhost+'/controleur.php?p=CouponsRemoveHome'; 
            axios.post(url,formData)
            .then((res)=>{
  
                    if(res.data == 'coupons-remove-home-success'){
        ...
        document.getElementById('loader').style.display='block';
          ...
          setTchek([]);
            ...
            for(var i=0 ;i <Tchek.length;i++){
                ...
                const url= localhost+'/controleur.php?p=deleteCoupons'; 
                axios.post(url,formData)
                .then((res)=>{
            ...
            <div> 
                 ...
                 <MetaTags>
                    ...
                    <div id='loader'>
                        ...
                        <div id='titre-admin'>
                            <h1><span className="glyphicon glyphicon-tags"  aria-hidden="true"></span><b>Coupons</b></h1>
                        </div>
                    ...
                    <div className="float-right" data-aos="zoom-in">
                                <Link to='/administration/coupons-add' className='btn btn-admin-add'>+</Link>
                    </div>
                </div> 
        ); 
  }
