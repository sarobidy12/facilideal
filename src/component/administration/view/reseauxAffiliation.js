...
const Click=()=>{
    ...
    const setelement= function setelement(e){
        ...
        document.getElementById('response').classList.replace('fade_up','fade_down');
            ...
            let formData= new FormData();
                ...
                const url= localhost+'/controleur.php?p=Viewaffilliation'; 
    ...
    const data= function data(){
        ...
        element.push(
            <div className='row table'>
                ...
                <div className='inline-table' style={{width:'20vh'}}>Titre</div>
                <div className='inline-table'></div>
             ...
             for(var i=0;i<resData.length;i++ ){
                 element.push( 
                    <div id={'del-'+resData[i].id} className='content-view-table row'>
                        ...
                        <div className='inline-table' style={{width:'20vh'}}>{resData[i].nom}</div>
                        <div className='inline-table'></div>
                        <div className='inline-table'>{resData[i].used}</div>
                        <div className='inline-table' style={{width:'10vh'}}>{gliphicon(resData[i].actif)}</div>  
                        <div className='inline-table' style={{width:'20vh'}}>{btnActive(resData[i].actif,resData[i].id)}</div>
                    </div>
                 )
             }       
    ...
    const activer = function activer(id,e){
        ...
        const url= localhost+'/controleur.php?p=AffilliationStatus'; 
        axios.post(url,formData)
        .then((res)=>{

                if(res.data == 'affilliation-status-home-success'){
                        setTchek([]);
                            if(cate === 0){
                                setelement(0);
                            }else{
        ...
        document.getElementById('loader').style.display='block';
          ...
          setTchek([]);
            ...
            for(var i=0 ;i <Tchek.length;i++){
                ...
                const url= localhost+'/controleur.php?p=deleteaffiliation'; 
                axios.post(url,formData)
                .then((res)=>{
                        if(res.data == 'affilliation-success'){
    ...
    return (
            <div> 
                 <MetaTags>
                    <title>Administration | affilliation </title>
                </MetaTags>
                    ...
                    <div id='titre-admin' >
                            <h1><span className="glyphicon glyphicon-random"  aria-hidden="true"></span><b>affilliation</b></h1>
                    </div>
                    ...
                    <div class="float-right" data-aos="zoom-in">
                        <Link to='/administration/reseaux-add' className='btn btn-admin-add'>+</Link>
                    </div>
                </div> 
        ); 
  }
